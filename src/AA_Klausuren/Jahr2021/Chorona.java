...
import static A_19_IO_Files.InputOutputControl.*;
...
public class Chorona {
  ...
  public static void main(String[] args) {
    try {
      ...
      int noOfPolluters = 6;
      /*
           int width = 7 + (int) Math.random() * 2;
      int height = 7 + (int) Math.random() * 2;
      int noOfPolluters =  5 + (int) Math.random()*2;
       */
  ...
  private static Map<Variant, Double> getPollutantFactors() {
    ...
    if (map.size() > 2) {
      ...
      while (map.size() > 2) map.remove(keys.remove(0));
    }

    return map;
  }

  private static void parsePolluteFactor(String line, Map<Variant, Double> map) {
  ...
  public static Color getColorForDose(double dose) {
    ...
    if (dose < limit1) return new Color(255, val, val);
    if (dose >= limit2) return Color.GREEN; // new Color( 216, 216, 216 );
    return new Color(160, 32, 240);
  }

  static void updateButtonForDose(JButton btn, double dose) {
    final int limit1 = 200;
    final int limit2 = 500;
    int val = Math.max(0, 255 - (int) (255 * (dose / limit1)));
    if (dose < limit1) {
